...
final class DurakGame {
    ...
    DurakGame(){reset();}
    static int suit(int c){return c%4;} static int rank(int c){return c/4;} static String card(int c){if(c<0)return "";String[]r={"6","7","8","9","10","J","Q","K","A"};return r[rank(c)]+new String[]{"♠","♥","♦","♣"}[suit(c)];}
    ...
    boolean addAttack(int p,int c){if(finished||p!=attacker||!hand[p].contains(c)||c<0)return false;if(attack.size()>=6)return false;if(!attack.isEmpty()){boolean ok=false;for(int a:attack)if(rank(a)==rank(c))ok=true;for(int d:defense)if(d>=0&&rank(d)==rank(c))ok=true;if(!ok)return false;}hand[p].remove((Integer)c);attack.add(c);defense.add(NONE);return true;}
    ...
    boolean take(int p){if(finished||p!=defender||attack.isEmpty())return false;for(int c:attack)hand[p].add(c);attack.clear();defense.clear();refill(attacker);refill(defender);int old=attacker;attacker=defender;defender=old;checkFinish();return true;}
    ...
    void checkFinish(){if(!deck.isEmpty())return;for(int p=0;p<2;p++)if(hand[p].isEmpty()){finished=true;loser=1-p;}}
