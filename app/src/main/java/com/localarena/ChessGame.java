...
final class ChessGame {
    ...
    List<Integer> pseudo(int from,boolean attacks){ArrayList<Integer> o=new ArrayList<>();char p=b[from];if(p==E)return o;boolean w=isW(p);int x=f(from),y=r(from);switch(Character.toLowerCase(p)){
      case 'p': {int d=w?-1:1, nr=y+d;if(!attacks&&nr>=0&&nr<8){int t=sq(x,nr);if(b[t]==E){o.add(t);int sy=w?6:1;if(y==sy&&b[sq(x,y+2*d)]==E)o.add(sq(x,y+2*d));}}for(int dx:new int[]{-1,1}){int nx=x+dx; if(nx<0||nx>7||nr<0||nr>7)continue;int t=sq(nx,nr);if(attacks||t==ep||(b[t]!=E&&!side(b[t],w)&&Character.toLowerCase(b[t])!='k'))o.add(t);}break;}
      ...
      case 'k': for(int dx=-1;dx<=1;dx++)for(int dy=-1;dy<=1;dy++)if(dx!=0||dy!=0)add(o,x+dx,y+dy,w,attacks);if(!attacks&&!check(w)&&((w&&x==4&&y==7)||(!w&&x==4&&y==0))){if(w&&wK&&b[sq(7,7)]=='R'&&b[sq(5,7)]==E&&b[sq(6,7)]==E&&!attacked(sq(5,7),false)&&!attacked(sq(6,7),false))o.add(sq(6,7));if(w&&wQ&&b[sq(0,7)]=='R'&&b[sq(1,7)]==E&&b[sq(2,7)]==E&&b[sq(3,7)]==E&&!attacked(sq(3,7),false)&&!attacked(sq(2,7),false))o.add(sq(2,7));if(!w&&bK&&b[sq(7,0)]=='r'&&b[sq(5,0)]==E&&b[sq(6,0)]==E&&!attacked(sq(5,0),true)&&!attacked(sq(6,0),true))o.add(sq(6,0));if(!w&&bQ&&b[sq(0,0)]=='r'&&b[sq(1,0)]==E&&b[sq(2,0)]==E&&b[sq(3,0)]==E&&!attacked(sq(3,0),true)&&!attacked(sq(2,0),true))o.add(sq(2,0));}break; }
      return o; }
    void add(List<Integer>o,int x,int y,boolean own,boolean attacks){if(x<0||x>7||y<0||y>7)return;int t=sq(x,y);if(attacks||b[t]==E||(!side(b[t],own)&&Character.toLowerCase(b[t])!='k'))o.add(t);}
    void slide(List<Integer>o,int x,int y,boolean own,int[][]ds,boolean attacks){for(int[]d:ds){int nx=x+d[0],ny=y+d[1];while(nx>=0&&nx<8&&ny>=0&&ny<8){int t=sq(nx,ny);if(b[t]==E)o.add(t);else{if((attacks||!side(b[t],own))&&!( !attacks&&Character.toLowerCase(b[t])=='k'))o.add(t);break;}nx+=d[0];ny+=d[1];}}}
