...
public class LogicSimulationTest {
    ...
    @Test public void seaBattleFullSmoke(){
        ...
        String encoded=s.encodeFor(1);assertEquals(3,encoded.split("\\|",-1).length);SeaBattleGame replica=new SeaBattleGame();replica.decode(encoded);assertTrue(replica.ready[0]&&replica.ready[1]);assertTrue(replica.cells[1][0][0]>=0&&replica.cells[1][0][0]<=3);
        int guard=0;
